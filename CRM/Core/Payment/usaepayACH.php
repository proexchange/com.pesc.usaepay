...
class CRM_Core_Payment_usaepayACH extends CRM_Core_Payment {
  /**
  ...
  function __construct( $mode, &$paymentProcessor ) {
    ...
    $this->_processorName    = ts('USAePayACH');
  }
  ...
  static function &singleton( $mode, &$paymentProcessor ) {
      ...
      if (self::$_singleton[$processorName] === null ) {
          self::$_singleton[$processorName] = new CRM_Core_Payment_usaepayACH( $mode, $paymentProcessor );
      }
  ...
  function doDirectPayment(&$params) {
    ...
    $tran->account = $params['bank_account_number'];
    $tran->routing = $params['bank_identification_number'];

    $tran->amount = $params['amount'];
    //$tran->cardholder = implode(' ', $fullname);
    $tran->cardholder = $params['account_holder'];
    ...
    if(0){
      echo "<pre><strong>PARAMS</strong><br>";
      print_r($params);
  ...
  /**
   * Sets appropriate parameters for checking out to usaepayACH
   *
